...
export const BlockPuzzleDuel: React.FC = () => {
  ...
  const [recoveredMatch, setRecoveredMatch] = useState<any | null>(null);
  ...
  const updateDragFromPointer = (index: number, clientX: number, clientY: number) => {
    ...
    // Keep the exact point where the user grabbed the tray piece. The floating
    // piece therefore follows the finger with the same offset instead of moving
    // its centre under the finger.
    const grabX = session?.grabX ?? 0;
    const grabY = session?.grabY ?? 0;
    const left = clientX - grabX;
    const top = clientY - grabY;
    // dragPointer now stores the floating piece's exact top-left position.
    // No centering transform is used, so the visual position and drop position
    // are the same coordinate system.
    setDragPointer({ x: left, y: top });

    // The drop target is calculated from the floating piece's VISUAL top-left,
    // not from the finger. This preserves the intentional drag offset all the
    // way through release and makes the placed blocks match what was shown.
    ...
    const stepX = secondRect && firstRect ? secondRect.left - firstRect.left : cellW;
    const stepY = gridRef.current.querySelector<HTMLElement>('[data-row="1"][data-col="0"]') && firstRect
      ? (gridRef.current.querySelector<HTMLElement>('[data-row="1"][data-col="0"]')!.getBoundingClientRect().top - firstRect.top)
      : cellH;
    const startCol = Math.round((left - (firstRect?.left ?? rect.left)) / stepX);
    const startRow = Math.round((top - (firstRect?.top ?? rect.top)) / stepY);
  ...
  return (
    <div className={`${isPlayingScreen ? 'block-gameplay-locked' : 'min-h-screen'} bg-[#070b18] text-white flex flex-col justify-between max-w-md mx-auto relative select-none`}>
      ...
      {/* 1. LOBBY VIEW */}
      {screenState === 'lobby' && recoveredMatch && !recoveredMatch.tournamentId && (
        <div className="mx-3 mt-3 p-3 rounded-2xl border border-amber-500/40 bg-amber-500/10 text-xs text-amber-100">
          <div className="font-black mb-1">আগের Block Puzzle ম্যাচটি অসম্পূর্ণ আছে</div>
          <div className="text-amber-200/80 mb-2">ব্রাউজার রিফ্রেশের কারণে গেমটি আর Resume করা যাবে না। এন্ট্রি ফি ফেরত নিতে নিচের বোতাম চাপুন।</div>
          <button
            type="button"
            className="w-full py-2 rounded-xl bg-amber-500 text-slate-950 font-black"
            onClick={async () => {
              const r = await refundBlockPuzzleMatch(String(recoveredMatch.id), Number(recoveredMatch.entryFee || 0), 'Browser refresh recovery');
              if (!r.success) { alert(r.message); return; }
              setRecoveredMatch(null);
              setActiveMatchId('');
            }}
          >
            এন্ট্রি ফি রিফান্ড করুন (৳{Number(recoveredMatch.entryFee || 0)})
          </button>
        </div>
      )}
      ...
      {screenState === 'playing' && (
        <div className="block-gameplay-stage px-3 py-2 space-y-2 relative flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          {dragPointer && activePiece && (
            <div
              className="fixed pointer-events-none z-50 opacity-90 scale-110 drop-shadow-2xl"
              style={{
                left: `${dragPointer.x}px`,
                // dragPointer is the exact visual top-left of the floating piece.
                // Its original grab offset is preserved above.
