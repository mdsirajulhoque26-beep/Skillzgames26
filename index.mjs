...
function maybeAwardReferralBonus(db, referredUserId) {
  ...
  if (approvedDeposit < settings.minDeposit || (settings.requireFirstProMatch && !firstProMatchPlayed)) {
    if (!record) { record = { id: id('ref'), referrerId: referrer.id, referredUserId: referred.id, referrerName: referrer.name, referredUserName: referred.name, bonusAmount: settings.bonusAmount, status: 'PENDING', createdAt: now() }; db.referrals.unshift(record); }
    return null;
  }
  const bonus = settings.bonusAmount;
  referrer.gamingBalance = money(Number(referrer.gamingBalance || 0) + bonus);
  record = record || { id: id('ref'), createdAt: now() };
  Object.assign(record, { referrerId: referrer.id, referredUserId: referred.id, referrerName: referrer.name, referredUserName: referred.name, bonusAmount: bonus, status: 'PAID', paidAt: now(), qualifyingDeposit: money(approvedDeposit) });
  db.referrals = [record, ...db.referrals.filter(r => r.id !== record.id)];
  db.transactions.unshift(makeTransaction(referrer.id, 'referral_bonus', bonus, 'Referral Bonus', `Gaming Balance • ${referred.name}`, 'referral', { referralId: record.id, referredUserId: referred.id }));
  return record;
}
...
app.patch('/api/deposit-requests/:id', auth, admin, async (req, res) => {
  ...
  if (r.status !== 'PENDING') return res.status(409).json({ message: 'Request already settled.' });
       if (status === 'APPROVED' && !r.reviewedAt) return res.status(409).json({ message: 'Withdrawal approval-এর আগে User Audit Review সম্পন্ন করতে হবে।' });
  ...
  r.status = status; r.adminNote = safeText(req.body?.reason || req.body?.note, 300);
  if (status === 'APPROVED') {
    const u = req.db.users.find(x => x.id === r.userId); if (!u) return res.status(404).json({ message: 'User not found' });
    u.gamingBalance = money(u.gamingBalance + Number(r.amount)); r.applied = true;
    maybeAwardReferralBonus(req.db, u.id);
    req.db.transactions.unshift(makeTransaction(u.id, 'deposit', Number(r.amount), 'Deposit Approved', `via ${r.method} • Admin Approved`, 'deposit', { trxId: r.trxId, paymentMethod: r.method }));
  } else {
    req.db.transactions.unshift({ ...makeTransaction(r.userId, 'deposit', 0, 'Deposit Rejected', r.adminNote || 'Rejected by Admin', 'deposit', { trxId: r.trxId }), status: 'REJECTED' });
  }
  await saveDb(req.db); res.json({ request: r, user: publicUser(req.db.users.find(x => x.id === r.userId)) });
});
